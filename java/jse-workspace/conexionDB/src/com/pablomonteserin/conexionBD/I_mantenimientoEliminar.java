...
public class I_mantenimientoEliminar {
	...
	public void eliminarRegistro(String dni, String nombre, String direccion){
		try {
			Connection conn = getConnection();
			Statement stm = conn.createStatement();
			stm.executeUpdate("DELETE FROM prueba WHERE dni=3");
			stm.close();
			conn.close();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {		
		I_mantenimientoEliminar mantenimientoEliminar = new I_mantenimientoEliminar();
		mantenimientoEliminar.eliminarRegistro("dni9", "nombre1", "direccion1");
	}
