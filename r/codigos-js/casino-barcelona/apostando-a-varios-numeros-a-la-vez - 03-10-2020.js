			...
			if (letreroTxt == letreroParaApostar) {
				if (estado == 'esperandoParaApostar' && opcionQueSalio != null) {
					...
					if (cantidadAApostar > stoploss ) {
						...
						console.log('Cantidad a apostar sin Martingala:' + cantidadAAPostarTotal);

						// Este código es el que incrementa la apuesta en función de si usamos Martingala
						for(let i = 0; i<numeroDeVecesQuePerdimos; i++){
							cantidadAAPostarTotal *= 2;
						}
						...
						console.log('Cantidad a apostar con Martingala:' + cantidadAAPostarTotal);
						console.log(plantillaArray);
						console.log('-------------------------------------------------');
