...
var letrerosParaEsperar = ['APUESTAS CERCA DEL CIERRE', 'NO HAY MÁS APUESTAS', 'ESPERANDO LA SIGUIENTE PARTIDA', 'APUESTAS ACEPTADAS', 'CERRANDO APUESTAS', 'APUESTAS CERRADAS', 'NO MORE BETS', 'APUESTAS RECHAZADAS'];
...
var repetidos = [];
var estaApuestaSeraALos4Ultimos = false;
...
var seleccionarOpcionApuesta = function () {
	console.log('entra en seleccionarOpcionApuesta')
	if(estaApuestaSeraALos4Ultimos ){
		opcionEscogida = repetidos;
	}else{
		if (colorQueSalio.toLowerCase() != 'verde') {
			opcionEscogida = colorQueSalio;
		}	
	}
...
var apostar = function (opcionEscogida) {
	...
	console.log('estaApuestaSeraALos4Ultimos')
console.log(estaApuestaSeraALos4Ultimos)
if(estaApuestaSeraALos4Ultimos){
	console.log(opcionEscogida);
	console.log("la opcion Escogida: " + opcionEscogida[0])
	btnParaApostarEscogido = getBotonParaApostarEnFuncionDelLaOpcionALaQueApostar(opcionEscogida[0]);
	...
	if (juegoActivo) {
		...
		if (estadoLetrero != null) {
			...
			if (letreroTxt == letreroParaApostar) {
				if (estado == 'esperandoParaApostar' && colorQueSalio != null) {
					primeraVez = false;
					...
					if (cantidadAApostar > stoploss) {
						...
						if(estaApuestaSeraALos4Ultimos){
							...
							var cantidadAApostarAux = cantidadAApostar*4;
							console.log('Cantidad a apostar:' + cantidadAApostarAux + ' €. =================== Array de apuestas: ' + apuestas.join(' -- '));
				...
				if (!letrerosParaEsperar.includes(letreroTxt) && estado == 'esperandoParaVerSiGane') {
					...
					gestionRepetidos(numeroQueSalio, colorQueSalio);

					if (!primeraVez) {

						if (evaluarApuesta(colorQueSalio, opcionEscogida)) {
							console.log("APUESTA GANADA. Salió: " + colorQueSalio + ", Opcion para apostar escogida: " + opcionEscogida + ' Cantidad ganada: ' + cantidadAApostar + ' €');
							...
							console.log("APUESTA PERDIDA. Salió: " + colorQueSalio + ", Aposté por: " + opcionEscogida + '. Cantidad Perdida: ' + cantidadAApostar + ' €');
							primeraVez = false;
						}
						estaApuestaSeraALos4Ultimos = false;
						console.log('YA ES POSIBLE PULSAR EL BOTÓN');
						++totalTiradas;
					}
...
var evaluarApuesta = (opcionQueSalio, opcionEscogida) => {
	if ( !estaApuestaSeraALos4Ultimos && opcionQueSalio.toLowerCase() == opcionQueSalio.toLocaleLowerCase()
	|| estaApuestaSeraALos4Ultimos && numeroQueSalio == opcionEscogida
	) {
		if (apuestas.length >= 2) {
			apuestas.splice(0, 1);
...
myBtn.setAttribute('id', 'apostarUltimos4');
myBtn.setAttribute('style', 'position:fixed; bottom: 30px; left: 0; width:100px; height:100px; background:blue; z-index:999');
myBtn.textContent = "Apostar a los últimos 4";
document.querySelector('body').append(myBtn);

myBtn.addEventListener('click', () =>{
	console.log('BOTON DE 4 ULTIMOS PULSADO');
	estaApuestaSeraALos4Ultimos = true;
});
