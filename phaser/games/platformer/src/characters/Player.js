class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    if (this.isAlive) {
      if (!this.isDazing) {
        ...
        if (
          ...
          this.getData("horizontalDirection") === Phaser.LEFT
        ) {
          this.leftWalk();
        } else if (
          this.cursors.right.isDown ||
          this.getData("horizontalDirection") === Phaser.RIGHT
        ) {
          this.rightWalk();
        } else {
          this.idle();
  ...
  checkEnemy(player, enemy) {
    ...
    if (player.body.touching.down && enemy.body.touching.up && !this.onGround) {
      enemy.die();
    } else {
      this.die();
    }
